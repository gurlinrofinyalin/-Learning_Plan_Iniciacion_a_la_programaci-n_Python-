...
df_train_eval = df.dropna(
    subset=[target_regresion]).copy()  # Elimina filas con NaN en el objetivo para entrenamiento/evaluación
...
scaler = preprocessing.StandardScaler()  # crea la herramienta para la estandarización, objeto scaler
"""
...
"""
####realiza dos operaciones consecutivas sobre tus datos de características (X) utilizando el objeto scaler que creaste.
...
# --- FUNCIONES PARA LAS MÉTRICAS PERSONALIZADAS (RAE, RSE) ---
# Estas funciones se crean para replicar las fórmulas mostradas en las imágenes
def relative_absolute_error(y_true, y_pred):
    """Calcula el Error Absoluto Relativo (RAE)."""
    # Numerador: Suma de los errores absolutos de las predicciones del modelo
    sum_abs_error_model = np.sum(np.abs(y_true - y_pred))
    # Denominador: Suma de los errores absolutos del modelo simple (la media)
    sum_abs_error_simple = np.sum(np.abs(y_true - np.mean(y_true)))

    # Manejar el caso de división por cero
    if sum_abs_error_simple == 0:
        return np.nan  # Retorna NaN si no hay variabilidad en y_true

    return sum_abs_error_model / sum_abs_error_simple


def relative_squared_error(y_true, y_pred):
    """Calcula el Error Cuadrático Relativo (RSE)."""
    # Numerador: Suma de los errores al cuadrado de las predicciones del modelo
    sum_sq_error_model = np.sum(np.square(y_true - y_pred))
    # Denominador: Suma de los errores al cuadrado del modelo simple (la media)
    sum_sq_error_simple = np.sum(np.square(y_true - np.mean(y_true)))

    # Manejar el caso de división por cero
    if sum_sq_error_simple == 0:
        return np.nan  # Retorna NaN si no hay variabilidad en y_true

    return sum_sq_error_model / sum_sq_error_simple
...
for train_index, test_index in kf.split(X_scaled):
    ...
    # --- CÁLCULO Y EVALUACIÓN DE TODAS LAS MÉTRICAS ---
    ...
    # RAE (Relative Absolute Error) - Usamos la función personalizada
    rae_fold = relative_absolute_error(y_test_fold, y_pred_fold)
    rae_scores.append(rae_fold)

    # RSE (Relative Squared Error) - Usamos la función personalizada
    rse_fold = relative_squared_error(y_test_fold, y_pred_fold)
    rse_scores.append(rse_fold)

    # R-cuadrado (R-squared) - También disponible en scikit-learn
    r2_fold = r2_score(y_test_fold, y_pred_fold)
    r2_scores.append(r2_fold)

    print(f"  MAE para el Fold {fold_num}: {mae_fold:.2f}")
    print(f"  MSE para el Fold {fold_num}: {mse_fold:.2f}")
    print(f"  RMSE para el Fold {fold_num}: {rmse_fold:.2f}")
    print(f"  RAE para el Fold {fold_num}: {rae_fold:.2f}")
    print(f"  RSE para el Fold {fold_num}: {rse_fold:.2f}")
    print(f"  R-cuadrado para el Fold {fold_num}: {r2_fold:.2f}")
...
print(f"MAE promedio en {n_splits} folds: {np.mean(mae_scores):.2f} +/- {np.std(mae_scores):.2f}")
print(f"MSE promedio en {n_splits} folds: {np.mean(mse_scores):.2f} +/- {np.std(mse_scores):.2f}")
print(f"RMSE promedio en {n_splits} folds: {np.mean(rmse_scores):.2f} +/- {np.std(rmse_scores):.2f}")
print(f"RAE promedio en {n_splits} folds: {np.mean(rae_scores):.2f} +/- {np.std(rae_scores):.2f}")
print(f"RSE promedio en {n_splits} folds: {np.mean(rse_scores):.2f} +/- {np.std(rse_scores):.2f}")
...
plt.scatter(df_train_eval['ENGINESIZE'], df_train_eval['CO2EMISSIONS'], color='blue',
            label='Datos reales')  # Dibuja los puntos de datos reales en azul.
# Superpone las predicciones del modelo como puntos rojos.
# Nota: Aquí transformamos todos los datos originales para poder trazar la línea de regresión del último modelo entrenado.
plt.plot(df_train_eval['ENGINESIZE'], model.predict(scaler.transform(df_train_eval[features_regresion].values)), 'ro',
         alpha=0.6, label='Predicciones del último modelo')
